...
# but making it fresh each time ensures a clean state and download from HF.
rm -rf "${SLURM_TMPDIR}/.cache/huggingface"
...
# Reduced segment length
MAX_SEG_LEN=5

echo "Training Resume Configuration:"
...
# ----- NEW: DOWNLOAD LAST CHECKPOINT FROM HF -----
echo "Fetching latest checkpoint from HF to resume training..."
if [ -n "$HF_REPO_ID" ]; then
    python3 download_last_checkpoint.py \
        --repo-id "$HF_REPO_ID" \
        --lang "$LANG" \
        --save-dir "$PT_SAVE_DIR"
else
    echo "Warning: HF_REPO_ID is empty. Skipping HF checkpoint download."
fi
# -------------------------------------------------
