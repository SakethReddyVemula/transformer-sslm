...
def generate(beam_ids, dict, eoms, prompt_len):
    # Replace pad
...
class SubwordSegmentalTextGenerator(nn.Module):
    def __init__(
        self,
        models,
        tgt_dict,
    ...
    def _generate(
        ...
        con_beam_eoms = [[[prompt_len - 1] for _ in range(beam_size)] for _ in range(bsz)]
        end_beam_eoms = [[[prompt_len - 1] for _ in range(beam_size)] for _ in range(bsz)]
        doprint = True
            ...
            with torch.autograd.profiler.record_function(
                ...
                if step == 0:
                    ...
                    for batch_num in range(bsz):
                        ...
                        prompt_ids = beam_ids[batch_num].unsqueeze(0)

                        for index in torch.topk(batch_con_lprobs, k=beam_size).indices:
                            con_beam_ids[batch_num].append(torch.cat([prompt_ids, torch.tensor([[index]]).to(device)], dim=1))
                        ...
                        for beam_num, index in enumerate(torch.topk(batch_end_lprobs, k=beam_size).indices):
                            end_beam_ids[batch_num].append(torch.cat([prompt_ids, torch.tensor([[index]]).to(device)], dim=1))
                            end_beam_lprobs[batch_num].append(torch.tensor([batch_end_lprobs[index].unsqueeze(-1)]).to(device))
                            end_beam_eoms[batch_num][beam_num].append(prompt_ids.size(1))
                    ...
                    con_history_embeddings = con_history_embeddings.view(active_bsz * beam_size, prompt_len + step - 1, -1)
                    ...
                    prev_end_eoms = [end_beam_eoms[batch_num][beam_num][-1] for batch_num in range(active_bsz)
                                     for beam_num in range(beam_size)]
                    end_history_embeddings = end_history_embeddings.view(active_bsz * beam_size, prompt_len + step - 1, -1)
                    ...
                    con_history_embeddings = con_history_embeddings.view(active_bsz, beam_size, prompt_len + step + 1 - 1, -1)
                    ...
                    end_history_embeddings = end_history_embeddings.view(active_bsz, beam_size, prompt_len + step + 1 - 1, -1)
                    ...
                    for batch_num in range(active_bsz):
                        ...
                        for beam_num in range(cand_size):
                            ...
                            if max_con_con_lprob > max_end_con_lprob:
                                ...
                                new_con_beam_ids[batch_num].append(torch.cat([beam_max_con_con_ids, torch.tensor([[max_con_con_id]]).to(device)], dim=-1))
                                ...
                                new_con_beam_ids[batch_num].append(torch.cat([beam_max_end_con_ids, torch.tensor([[max_end_con_id]]).to(device)], dim=-1))
                        ...
                        for beam_num in range(cand_size):
                            ...
                            if max_con_end_lprob > max_end_end_lprob:
                                ...
                                new_end_beam_ids[batch_num].append(
                                    torch.cat([beam_max_con_end_ids, torch.tensor([[max_con_end_id]]).to(device)], dim=-1))
                                ...
                                beam_max_con_end_eoms.append(prompt_len + step)
                                ...
                                new_end_beam_ids[batch_num].append(
                                    torch.cat([beam_max_end_end_ids, torch.tensor([[max_end_end_id]]).to(device)], dim=-1))
                                ...
                                beam_max_end_end_eoms.append(prompt_len + step)
                    ...
                    for batch_num in range(active_bsz):
                        for beam_num in range(beam_size):
                            if step == max_len:
                                # print(con_beam_ids)
                                break
                            generate(con_beam_ids[batch_num][beam_num], self.tgt_dict, con_beam_eoms[batch_num][beam_num], prompt_len)
                        print("------")
                        for beam_num in range(len(end_beam_ids[batch_num])):
                            generate(end_beam_ids[batch_num][beam_num], self.tgt_dict, end_beam_eoms[batch_num][beam_num], prompt_len)
                        print("-------------------------------------------------")
