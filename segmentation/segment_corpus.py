...
def get_model_segmentation(model, task, text_list, device, batch_size=512):
    ...
    i = 0
    pbar = tqdm(total=len(text_list), desc="Segmenting words", leave=False)
...
def get_checkpoints(args):
    ...
    candidates.sort(key=lambda f: get_ckpt_num(os.path.basename(f)))
    return [(os.path.basename(f), f) for f in candidates if get_ckpt_num(os.path.basename(f))[0] >= 0] 
...
def flush_uploads(repo_id, token, subfolder=None, delete_local=False, force=False, batch_size=3):
    ...
    try:
        ...
        api.create_commit(
            ...
            commit_message=f"Upload {len(files_to_upload)} corpus segmentations: {', '.join(filenames)}",
            repo_type="dataset"
        )
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="corpus_segmentations")
    ...
    print(f"Loaded {len(unique_words)} unique words from dataset for segmentation.")
    ...
    for ckpt_name, ckpt_ref in tqdm(checkpoints, desc="Checkpoints"):
        # Output filename as JSONL
        output_filename = f"seg_corpus_{args.lang}_{ckpt_name.replace('.pt', '.jsonl')}"
        ...
        try:
            ...
            # Segment all unique words
            segmentations_dict = get_model_segmentation(model, task, unique_words, device, batch_size=args.batch_size)
            
            # Reconstruct sentences and save in JSONL format
            print(f"Writing segmented corpus to {output_file}...")
            with open(data_file, 'r', encoding='utf-8') as f_in, open(output_file, 'w', encoding='utf-8') as f_out:
                for line in f_in:
                    words = line.strip().split()
                    # Map each word to its segmented list. Fallback to just the word if not segmented safely.
                    segmented_line = [segmentations_dict.get(w, [w]) for w in words]
                    # Write to JSONL
                    f_out.write(json.dumps(segmented_line, ensure_ascii=False) + '\n')
