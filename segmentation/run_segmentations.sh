#!/bin/bash
#SBATCH --job-name=seg4_sslm
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=10
#SBATCH --gres=gpu:1
#SBATCH --mem-per-cpu=2048
#SBATCH --time=2-00:00:00
#SBATCH --output=seg_sslm_%j.out
#SBATCH --error=seg_sslm_%j.err
...
echo "Validation Data Base: /home2/$USER/saketh/dataset/"
...
for LANG_CODE in "${LANGS[@]}"; do
    ...
    DATA_PATH=/home2/$USER/saketh/dataset/${LANG_CODE}/test.${LANG_CODE}
    if [ ! -f "$DATA_PATH" ]; then
        DATA_PATH=/home2/$USER/saketh/dataset/${LANG_CODE}/valid.${LANG_CODE}
    fi
